...
namespace ModForge.Shared.Models.ModItems
{
	public class Key : IModItem, IItem
	{
		...
		public Key(string id, string path, List<string> linkedIds, List<IAttribute> attributes, Localization localization)
		{
			Id = id;
			Path = path;
			LinkedIds = linkedIds;
		...
		{
			return new Key(Id, Path, LinkedIds, Attributes.Select(attr => attr.DeepClone()).ToList(), Localization.DeepClone());
		}
	}
}
