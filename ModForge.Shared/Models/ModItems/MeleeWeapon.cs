...
namespace ModForge.Shared.Models.ModItems
{
	public class MeleeWeapon : IModItem, IItem
	{
		...
		public MeleeWeapon(string id, string idKey, string path, List<string> linkedIds, List<IAttribute> attributes, Localization localization)
		{
			Id = id;
			IdKey = idKey;
			Path = path;
		...
		{
			return new MeleeWeapon(Id, IdKey, Path, LinkedIds, Attributes.Select(attr => attr.DeepClone()).ToList(), Localization.DeepClone());
		}
	}
}
