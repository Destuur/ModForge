...
namespace ModForge.Shared.Models.ModItems
{
	public class PickableItem : IModItem, IItem
	{
		...
		public PickableItem(string id, string path, List<string> linkedIds, List<IAttribute> attributes, Localization localization)
		{
			Id = id;
			Path = path;
			LinkedIds = linkedIds;
		...
		{
			return new PickableItem(Id, Path, LinkedIds, Attributes.Select(attr => attr.DeepClone()).ToList(), Localization.DeepClone());
		}
	}
}
