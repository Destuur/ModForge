...
namespace ModForge.Shared.Models.ModItems
{
	public class Armor : IModItem, IItem
	{
		...
		public Armor(string id, string idKey, string path, List<string> linkedIds, List<IAttribute> attributes, Localization localization)
		{
			Id = id;
			IdKey = idKey;
			Path = path;
		...
		{
			return new Armor(Id, IdKey, Path, LinkedIds, Attributes.Select(attr => attr.DeepClone()).ToList(), Localization.DeepClone());
		}
	}
}
