...
namespace ModForge.Shared.Models.ModItems
{
	public class Die : IModItem, IItem
	{
		...
		public Die(string id, string path, List<string> linkedIds, List<IAttribute> attributes, Localization localization)
		{
			Id = id;
			Path = path;
			LinkedIds = linkedIds;
		...
		{
			return new Die(Id, Path, LinkedIds, Attributes.Select(attr => attr.DeepClone()).ToList(), Localization.DeepClone());
		}
	}
}
