...
namespace ModForge.Shared.Models.ModItems
{
	public class NPCTool : IModItem
	{
		...
		public NPCTool(string id, string path, List<string> linkedIds, List<IAttribute> attributes, Localization localization)
		{
			Id = id;
			Path = path;
			LinkedIds = linkedIds;
		...
		{
			return new NPCTool(Id, Path, LinkedIds, Attributes.Select(attr => attr.DeepClone()).ToList(), Localization.DeepClone());
		}
	}
}
